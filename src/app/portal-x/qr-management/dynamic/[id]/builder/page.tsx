...
import {
    ArrowLeft, Undo2, Download, FileDown, Loader2, Type, ImageIcon, Crosshair, Monitor
} from 'lucide-react';
...
import { formatIdLabel } from '@/lib/qrCodeGenerator';
import api from '@/lib/api';
import type { DynamicQR } from '@/types/qr.types';

const QR_PAGE_SIZE_MM = 60;

function DynamicBuilderInner({ qr }: { qr: DynamicQR }) {
    ...
    const builder = useBuilderState(qr.label, qr.qr_url);
    ...
    const handleExportPNG = useCallback(async () => {
        setIsExporting(true);
        setExportLabel('Exporting PNG...');
        try {
            await builder.exportPNG(qr.label);
        } finally {
    ...
    const handleExportPDF = useCallback(async () => {
        setIsExporting(true);
        setExportLabel('Generating single-page PDF...');
        try {
            const canvas = await builder.renderToCanvas(2400);
            if (!canvas) return;
            ...
            pdf.save(`${qr.label.replace(/[\\/:*?"<>|]+/g, '-').trim() || 'dynamic-qr'}.pdf`);
        } finally {
            setIsExporting(false);
            setExportLabel('');
        }
...
export default function QRDynamicBuilderPage() {
    const params = useParams();
    const router = useRouter();
    const id = params?.id as string;

    const { data, isLoading, isError } = useQuery({
        queryKey: ['dynamic-qr-builder', id],
        queryFn: async () => {
            const response = await api.get<{ data: DynamicQR }>(`/admin/qr/dynamic/${id}`);
    ...
    if (isError || !data) {
        return (
            <div style={{ height: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#020617', color: 'white', margin: '-24px', gap: 16 }}>
                <p style={{ color: '#94a3b8' }}>Dynamic QR not found.</p>
                <button
                    onClick={() => router.back()}
    ...
    return <DynamicBuilderInner qr={data} />;
}
